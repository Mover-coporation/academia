...
class User extends CI_Controller {
	...
	function dashboard()
	{
        ...
        $schooid = $this->myschool['id'];

        //$data['schoolinfo']  = $info;
        #Fetch Student Information from Dashboard ...
        $data['activestudents'] = $this->Query_reader->get_row_as_array('count_student_list', array('isactive' => 'Y', 'limittext' =>'','searchstring'=> 'AND school ='.$schooid));
        $data['inactivestudents'] = $this->Query_reader->get_row_as_array('count_student_list', array('isactive' => 'N', 'limittext' =>'','searchstring'=> ' AND school ='.$schooid ));
        $search  = "usergroup in (select id from usergroups where groupname like 'Teacher%') and school = ".$schooid;
        $data['teachers'] = $this->Query_reader->get_row_as_array('count_school_users', array('isactive' => 'Y', 'limittext' =>'','searchstring'=> $search ));
        $data['classes'] = $this->Query_reader->get_row_as_array('count_classes', array('isactive' => 'Y', 'limittext' =>'','searchstring'=> 'AND school ='.$schooid ));
        $data['terms'] = $this->Query_reader->get_row_as_array('count_terms', array('isactive' => 'Y', 'limittext' =>'','searchstring'=> 'AND school ='.$schooid ));
        $data['users'] = $this->Query_reader->get_row_as_array('count_school_users', array('isactive' => 'Y', 'limittext' =>'','searchstring'=> '1 and 1 AND school ='.$schooid ));
        $data['lent']= $this->Query_reader->get_row_as_array('count_books_transacted',  array('isactive' => 'Y', 'limittext' =>'','searchstring'=> '1 and 1 AND status like  "OUT" AND  bookid in (select id from librarystock where school='.$schooid.' ) ' ));
        $data['defaulters']= $this->Query_reader->get_row_as_array('count_defaulters',  array('isactive' => 'Y', 'limittext' =>'','searchstring'=> '  returned = "N"  AND  school='.$schooid.'  ' ));
        $data['available']= $this->Query_reader->get_row_as_array('count_books_transacted',  array('isactive' => 'Y', 'limittext' =>'','searchstring'=> ' 1 AND 1  AND  bookid in (select id from librarystock where school = '.$schooid.') ' ));
        $data['total_credit']= $this->Query_reader->get_row_as_array('total_finances',  array('isactive' => 'Y', 'limittext' =>'','searchstring'=> ' type = "CREDIT" AND  school = '.$schooid.'' ));
        $data['total_debit']= $this->Query_reader->get_row_as_array('total_finances',  array('isactive' => 'Y', 'limittext' =>'','searchstring'=> ' type = "DEBIT" AND  school = '.$schooid.'' ));

        //count_books_transacted
        //SELECT COUNT(*) as availableBooks FROM librarytransactions WHERE transactiontype = _type_
//count_terms
	...
	function manage_staff_group_rights()
	{
		...
		if($this->input->post('updategrouppermissions'))
		{
			if(!empty($_POST['permissions'])){
				$groupid = decryptValue($_POST['editid']);
				
				$result_array = array();
	
				#First delete all permissions from the access table
				$delresult = $this->db->query($this->Query_reader->get_query_by_code('delete_group_permissions', array('groupid'=>$groupid)));
				
				array_push($result_array, $delresult);
				
				foreach($_POST['permissions'] AS $permissionid)
				{
					$insresult = $this->db->query($this->Query_reader->get_query_by_code('add_group_permission', array('groupid'=>$groupid, 'permissionid'=>$permissionid) ));
				...
				if(get_decision($result_array)){
					...
					$data['msg'] = "SUCCESS: The Group permissions have been assigned.";
		...
		if(empty($result) || !$result)
		{
			if(empty($_POST['permissions']))
			{
				...
				$data['msg'] = "WARNING: No permissions are assigned to the group.";
			}
			else
			{
				#$this->session->set_userdata('puser', "ERROR: The group permissions could not be assigned.");
				$data['msg'] = "ERROR: The group permissions could not be assigned.";
			}
