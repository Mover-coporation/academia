...
function student_photo_thumb($photo)
{
...
function get_subject_teachers($obj, $subject)
{	
	$query = "SELECT count(*) as numofteachers, teacher, subject FROM subjectteacherassignments WHERE subject = ".$subject." GROUP BY teacher, subject";
	return $obj->db->query($query);
}


#function to get the current term
function current_term($obj, $school)
{
	return $obj->Query_reader->get_row_as_array('get_current_term', array('school'=>$school));
}

#function to get the grade of a mark given the grading scale
function get_grade($grading_details, $mark)
{
	$mark_grade = array();
	foreach($grading_details as $grade_detail)
	{
		if($mark > $grade_detail['mingrade'] && $mark < $grade_detail['maxgrade'])
		{
			$mark_grade['grade'] = $grade_detail['symbol'];
			$mark_grade['value'] = $grade_detail['value'];
			break;
		}
	}
...
function gen_pdf($obj, $html,$paper='A4')
{
	$obj->load->library('mpdf56/mpdf');               
	$mpdf=new mPDF('utf-8',$paper);
	$mpdf->WriteHTML($html);
	$mpdf->Output();
}
// get all student perfomance per subject
function get_student_subject_info($obj,$class, $exam, $subject, $mark )
{
 if($exam=='' ) exit();
    $student_marks = $obj ->db->query('SELECT *  FROM marksheet WHERE class='.$class.' AND exam='.$exam.' AND isactive="Y" AND subject='.$subject.' AND mark >='.$mark)->result_array();

    return $student_marks;

}
//get subject performance summary for a given class, term and exam
function get_subject_performance_summary($obj, $class, $exam, $subject)
{
	$summary_array = array('average', 'highest', 'lowest', 'students','studentmarks');
    ...
    #Number of students
	$number_of_students = $obj->db->query('SELECT COUNT(*) AS numOfStudents FROM marksheet WHERE class='.$class.' AND exam='.$exam.' AND isactive="Y" AND subject='.$subject)->result_array();
		
	#Sum of marks
	$marks_sum_result = $obj->db->query('SELECT SUM(mark) AS sumOfMarks FROM marksheet WHERE class='.$class.' AND exam='.$exam.' AND isactive="Y" AND subject='.$subject)->result_array();
	
	//get the highest mark
	$max_mark_arr = $obj->db->query('SELECT MAX(mark) AS maxMark FROM marksheet WHERE class='.$class.' AND exam='.$exam.' AND isactive="Y" AND subject='.$subject)->result_array(); 
	
	//get the lowest mark
	$min_mark_arr = $obj->db->query('SELECT MIN(mark) AS minMark FROM marksheet WHERE class='.$class.' AND exam='.$exam.' AND isactive="Y" AND subject='.$subject)->result_array(); 
	
	if(empty($marks_sum_result[0]['sumOfMarks']))
	{
		$summary_array['average'] = '';
		$summary_array['highest'] = '';
		$summary_array['lowest'] = '';
		$summary_array['students'] = '';
        $summary_array['studentmarks'] ='';
	}
	else
	{
		$summary_array['average'] = number_format($marks_sum_result[0]['sumOfMarks']/$number_of_students[0]['numOfStudents'], 1);
		$summary_array['highest'] = $max_mark_arr[0]['maxMark'];
		$summary_array['lowest'] = $min_mark_arr[0]['minMark'];
		$summary_array['students'] = $number_of_students[0]['numOfStudents'];
     }
