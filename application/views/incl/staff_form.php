          ...
          <tr>
					 ...
					 if(!empty($isview))
			{
				echo "<span class='viewtext'>".$userdetails['telephone']."</span><input type='hidden' name='telephone' id='telephone' value='".$formdata['telephone']."' />";
				  ...
				  <tr>
                    <td>&nbsp;</td>
                    <td><input onClick="saveStaff('<?php echo ((!empty($i))? '/i/'.$i : '');?>', 'firstname<>*middlename<>*telephone<>lastname<>emailaddress<>*usergroup<>address<>username<>password<>repeatpassword<>save<?php echo (!empty($i) || !empty($editid))? '<>editid' : ''; ?>')" type="button" name="save" id="save" value="Save" class="button"/></td>
                    <td>&nbsp;</td>
                  </tr>
                  <?php } ?>
                </table>
