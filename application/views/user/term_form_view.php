...
<div id="term_results" class="content">
<?php
          ...
          <tr>
			...
			if(!empty($isview))
			{
				echo "<span class='viewtext'>".$userdetails['firstname']."</span>";
					...
					?>
                      <input type="text" name="term" id="term" class="textfield" size="30" <?php if(!empty($termdetails['term']))
				 	 echo 'value="' . $termdetails['term'] . '"'; ?>/>
                        ...
                        <?php if(empty($isview))
			{
				?>
				  ...
				  <tr>
                    ...
                    <td><input type="button" id="save" name="save" onclick="saveTerm('<?php echo ((!empty($i))? '/i/'.$i : '');?>', 'term<>year<>startdate<>enddate<>save<?php if(!empty($i) || !empty($editid)) echo '<>editid'?>');" value="Save" class="button"/></td>
                    </tr>
                  <?php } ?>
                </table>
